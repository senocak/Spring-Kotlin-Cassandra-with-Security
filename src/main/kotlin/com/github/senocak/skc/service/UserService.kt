...
import com.github.senocak.skc.domain.PasswordResetToken
import com.github.senocak.skc.domain.PasswordResetTokenRepository
import com.github.senocak.skc.domain.User
import com.github.senocak.skc.domain.UserRepository
import com.github.senocak.skc.domain.dto.ChangePasswordRequest
import com.github.senocak.skc.exception.ServerException
import com.github.senocak.skc.util.OmaErrorMessageType
import com.github.senocak.skc.util.RoleName
import com.github.senocak.skc.util.logger
import com.github.senocak.skc.util.randomStringGenerator
...
class UserService(
    ...
    fun findAllUsers(page: Int, size: Int): Slice<User> = userRepository.findAll(PageRequest.of(page, size))
    fun findAllUsers(): MutableList<User> = cassandraTemplate.select(Query.empty(), User::class.java)

    // FIXME: query is not working with like or contains
    fun createSpecificationForUserAndRetrieve(
        ...
        run {
            val listOfCriteria = arrayListOf<CriteriaDefinition>()
            if(name != null) listOfCriteria.add(element = Criteria.where("name").like("%$name%"))
            if(email != null) listOfCriteria.add(element = Criteria.where("email").contains(email))
            if(roleIds != null) listOfCriteria.add(element = Criteria.where("roles").`in`(roleIds))
            if(startDate != null || endDate != null) {
                if(startDate != null)
                    listOfCriteria.add(element = Criteria.where("createdAt").gte(LocalDateTime.parse(startDate, formatter)))
                if(endDate != null)
                    listOfCriteria.add(element = Criteria.where("createdAt").lte(LocalDateTime.parse(endDate, formatter)))
            }
            val query: Query = Query.query(listOfCriteria)
            cassandraTemplate.select(query.withAllowFiltering(), User::class.java)
            //cassandraOperations.query(User::class.java)
            //    .`as`(User::class.java)
            //    .matching(query.withAllowFiltering())
            //    .all()
        }

    fun getByName(name: String): List<User> =
        cassandraTemplate.select(Query.query(Criteria.where("name").`is`(name)).withAllowFiltering(), User::class.java)
    ...
    fun findByIdTemplate(id: UUID): User =
        cassandraOperations.selectOneById(query(where("id").`is`(id)), User::class.java)
